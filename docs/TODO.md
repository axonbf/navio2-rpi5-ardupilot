...
| 23 | Fix `--serial2 /dev/ttyAMA0` in service config | Medium | On Pi 5 the primary UART is `/dev/ttyAMA10` (symlink `/dev/serial0`); `/dev/ttyAMA0` does NOT exist. `/etc/default/ardurover` still has `--serial2 /dev/ttyAMA0` → RC receiver on serial2 fails silently. Fix when RC receiver is connected to Pi 5. |
...
| 25 | **Power: high-current supply + precise I2C measurement** (parts on order) | Medium | Plan chosen 2026-07-19 (supersedes the old "verify analog PM" item). **Supply:** Pololu 12A (D24V120F12) or Matek BEC12S PRO → 5 V to Pi 5 + Hailo stack (~6–8 A peak). **Measure:** INA226/INA228 I2C monitor on Navio2 I2C bus 1 (diff addr from MS5611), `BATT_MONITOR=21` (INA2xx), `BATT_I2C_BUS`/`BATT_I2C_ADDR`; shunt high-side on main battery + so it reads total draw. Far more precise than the analog PM; removes the "measure when not primary path" concern. User ordering parts to validate. Only needed when Hailo HAT is re-installed (now removed → 3 A analog PM suffices). See AGENTS "Power module" + SESSION_HISTORY Session 20. |
