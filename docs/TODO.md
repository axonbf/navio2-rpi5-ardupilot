...
| ~~11~~ | ✅ **DONE 2026-08-01** — ROS 2 **Humble** + ArduPilot native DDS **working** | Medium | Full as-built in **[`docs/ROS2_HUMBLE_DDS_PLAN.md`](ROS2_HUMBLE_DDS_PLAN.md)**. ArduPilot publishes its native `/ap/...` DDS topics to ROS 2 Humble; verified with live data (`/ap/navsat` GPS) on the Pi, laptop@home (LAN), and laptop over **WireGuard** (additive: local multicast + wg0 unicast). **Key findings:** (1) **UDP DDS is dead on native Navio2/Linux** — AP_DDS-UDP requires `AP::network().get_ip_active()!=0`, but Navio2 has **no AP_Networking backend** (only CHIBIOS/PPP/SITL), so it spins forever, never sends; `NET_ENABLE` can't help. **Serial (XRCE) transport is the only one that works** (it skips that check) → bridged same-machine over a **socat PTY pair**. (2) The **agent is Fast-DDS**, so all ROS 2 must be **Fast-DDS + domain 0** (earlier CycloneDDS/domain-42 plan abandoned). **Setup:** RoboStack ros-humble; `ardurover` rebuilt `--enable-DDS` (needs `microxrceddsgen`+JDK); agent = source-built **Micro-XRCE-DDS-Agent v2.4.2** (patched fastdds tag); 3 systemd units (`dds-pty` socat, `xrce-agent` serial, ardurover `After=dds-pty`); params `DDS_ENABLE=1 SERIAL5_PROTOCOL=45 SERIAL5_BAUD=115` + `--serial5 /dev/ttyDDS0`. Helpers: `~/ros2_env.sh` (Pi), `~/ros2_boat.sh` + `~/fastdds_wg.xml` (laptop, additive: local multicast + wg0 unicast). **Reboot-verified** — survives a clean boot (all units auto-start, agent reconnects, live GPS confirmed from laptop). **2026-08-02:** laptop profile changed from wg0-only to additive (local multicast `239.255.0.1` + Pi `10.0.0.5` unicast peer) — `initialPeersList` replaces Fast-DDS defaults, so multicast must be listed explicitly or local discovery is lost. Verified: two terminals both sourcing `ros2_boat.sh` now discover each other AND the Pi. |
...
4. ~~Full QGC calibration~~ — ✅ **DONE** (radio, modes, failsafe, servo direction; battery config optional while RC-only — see task #14).
...
7. ~~Install ROS2 + ArduPilot DDS~~ — ✅ **DONE 2026-08-01** (Humble + native DDS over serial-PTY; see #11). ~~Jazzy~~ — RoboStack has no aarch64 Jazzy build; Humble used instead.
