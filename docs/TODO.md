...
| 11 | Install ROS2 Jazzy + ArduPilot native DDS | Medium | **Plan ready → [`docs/ROS2_JAZZY_DDS_PLAN.md`](ROS2_JAZZY_DDS_PLAN.md)** (6 steps). **Run AFTER the lake test.** Decided 2026-07-31 (user+Claude): keep Pi OS (Navio2/RCIO already works; Ubuntu would force re-porting the kernel stack); vehicle = **RoboStack** (native ROS 2 on Debian, not Docker — Docker NAT breaks DDS discovery over WireGuard); integration = **native DDS (AP_DDS) + micro-ROS agent**, not MAVROS (MAVROS setpoint/RC rate-limited). Current `ardurover` has no AP_DDS → rebuild with `--enable-dds`. Goal: `ros2 topic list` from laptop over WireGuard (needs CycloneDDS unicast peers 10.0.0.5↔10.0.0.2). Disk OK (20 GB free). |
