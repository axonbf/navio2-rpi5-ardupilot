...
- **Envs:** Pi `source ~/ros2_env.sh`; laptop @home = default env works (Connext↔Fast-DDS over LAN); laptop remote (WireGuard) `source ~/ros2_boat.sh` (additive: local multicast + Pi `10.0.0.5` wg0 unicast, `~/fastdds_wg.xml` — `initialPeersList` replaces Fast-DDS defaults so multicast `239.255.0.1` is listed explicitly). AP-specific types (`ardupilot_msgs`) built in colcon ws `~/ap_ws` → `source ~/ap_ws/install/setup.bash`.
